...
mongoose.connect('mongodb+srv://test:test1234@cluster0.x43f49z.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0')
...
app.post("/createform", async (req, res) => {
  try {
    ...
    if (!hasFields && !hasSections) {
      return res.status(400).json({ error: "Form must have at least one section or field." });
    }

    if (hasFields) {
      normalizedFields = data.fields.map((f, i) => ({
        id: f.field_id ?? `field_${i + 1}`,
        label: f.label?.trim(),
        type: f.type,
    ...
    if (hasSections) {
      normalizedSections = data.sections.map((section, i) => ({
...
app.patch("/submitform", async (req, res) => {
  try {
    ...
    res.status(201).json({ message: 'Response updated', response: updatedResponse });
  } catch (err) {
...
app.get("/submitformbyid", async (req, res) => {
  try {
    ...
    const forms = await FormResponse.find({ _id: new mongoose.Types.ObjectId(responseId) });
    return res.status(200).json({ forms });
  } catch (err) {
    console.error("Error in GET /createform:", err);
    return res.status(500).json({ error: err.message });
